...
from transformers import Qwen2ForCausalLM as OrigQwen
...
def _normalize_config(cfg):
    ...
    # Qwen2Config has `sliding_window` (None when use_sliding_window=False) and
    # `layer_types` (defaulted from sliding_window during config init). We still
    # defensively backfill them in case an older checkpoint omits the field.
...
def main(args):
    ...
    orig_model = OrigQwen.from_pretrained(model_config._name_or_path,
                                          torch_dtype='auto',
                                          low_cpu_mem_usage=True,
                                          config=model_config)
