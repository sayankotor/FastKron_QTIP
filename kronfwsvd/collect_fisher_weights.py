"""
Collect per-step gradient samples for Fisher / Kronecker factor estimation.

Designed for chunked layer processing on large models (e.g. Qwen2.5-32B):
the script unfreezes only nn.Linear projections in [--layer_start, --layer_end)
that match --layer_pattern, then runs HF Trainer for one epoch (or --max_steps
optimizer steps). Gradients of unfrozen layers are saved to
{grad_dir}/grad_step_{N:04d}.safetensors as bfloat16, one file per
gradient_accumulation_steps cycle.

Note on semantics: per-step "gradients" are NOT full accumulated gradients
across all microbatches. optimizer.step/zero_grad run on every microbatch
(not gated by accum-cycle), so the saved tensor is the LAST microbatch's
grad, scaled by 1/gradient_accumulation_steps. This sampling-style behavior
is inherited from the pre-existing 7B pipeline and is intentionally preserved.
...
class CustomTrainer(Trainer):
    ...
    def training_step(
        ...
        # NOTE: optimizer.step / zero_grad / lr_scheduler.step run on EVERY
        # microbatch, not gated by the accumulation cycle. Inherited from the
        # 7B-validated pipeline; means saved gradients are samples of the
        # last microbatch's grad in each cycle, scaled by 1/accum, not full
        # accumulated gradients. Intentionally preserved.
        self.optimizer.step()
...
def training_process(args) -> None:
    print(
        f"[{stamp()}] [collect] starting | model={args.model_name} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect per-step gradient samples for Kronecker factor estimation."
    )
    parser.add_argument("--model_name", type=str, required=True,
                        help="HF model identifier (e.g. Qwen/Qwen2.5-32B)")
    ...
    parser.add_argument("--grad_accum", type=int, default=384,
                        help="gradient_accumulation_steps; default 384 → "
                             "100 step-files on default 38400-sample subset")
