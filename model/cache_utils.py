...
class CacheConfig:
    ...
    def update(self, **kwargs):
        ...
        unused_kwargs = {
            key: value
            for key, value in kwargs.items() if key not in to_remove
        }
...
class DynamicCache(Cache):
    ...
    def update(
        ...
        if key_states is None or value_states is None:
            # Если данных нет, просто возвращаем текущее состояние слоя (или пустые списки)
        ...
        ) == 0:  # fills previously skipped layers; checking for tensor causes errors
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        layer_seq_length = self.key_cache[layer_idx].shape[
            -2] if not is_empty_layer else 0
    ...
    def from_legacy_cache(cls,
                          past_key_values: Optional[Tuple[Tuple[
                              torch.FloatTensor]]] = None,
                          num_hidden_layers: int = None) -> "DynamicCache":
    ...
    def batch_split(self,
                    full_batch_size: int,
                    split_size: int,
                    num_hidden_layers: int = None) -> List["DynamicCache"]:
    ...
    @classmethod
    @deprecate_kwarg("num_hidden_layers", version="4.47.0")
    def from_batch_splits(cls,
                          splits: List["DynamicCache"],
                          num_hidden_layers: int = None) -> "DynamicCache":
        ...
        for idx in range(len(splits[0])):
            key_cache = [
                current.key_cache[idx] for current in splits
    ...
    def batch_repeat_interleave(self, repeats: int):
        ...
        for layer_idx in range(len(self)):
            self.key_cache[layer_idx] = self.key_cache[
                layer_idx].repeat_interleave(repeats, dim=0)
            self.value_cache[layer_idx] = self.value_cache[
                layer_idx].repeat_interleave(repeats, dim=0)
...
class OffloadedCache(DynamicCache):
    ...
    def evict_previous_layer(self, layer_idx: int):
        ...
        if len(self) > 2:
            ...
            self.value_cache[prev_layer_idx] = self.value_cache[
                prev_layer_idx].to("cpu", non_blocking=True)
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self.key_cache[layer_idx] = torch.cat([key_tensor, key_states],
                                                  dim=-2)
            self.value_cache[layer_idx] = torch.cat(
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self._quantized_value_cache.append(
                self._quantize(value_states.contiguous(),
            ...
            keys_to_return = [
                dequant_key, self.key_cache[layer_idx], key_states
            ]
            values_to_return = [
                dequant_value, self.value_cache[layer_idx], value_states
            ]

            keys_to_return = torch.cat(keys_to_return, dim=-2)
            values_to_return = torch.cat(values_to_return, dim=-2)
            if (self.key_cache[layer_idx].dim() == 4
                    and self.key_cache[layer_idx].shape[-2] + 1
...
class QuantoQuantizedCache(QuantizedCache):
    ...
    def _quantize(self, tensor, axis):
        scale, zeropoint = self.optimizer(tensor, self.qtype.bits, axis,
                                          self.q_group_size)
        qtensor = AffineQuantizer.apply(tensor, self.qtype, axis,
                                        self.q_group_size, scale, zeropoint)
        return qtensor
...
class HQQQuantizedCache(QuantizedCache):
    ...
    def _quantize(self, tensor, axis):
        ...
        self.quantizer.cuda(
            qtensor, meta=meta,
            device=self.device)  # Move to device and cast to dtype
        return qtensor, meta
        ...
        if using_rope and layer_idx == 0:
            ...
            if cos.dim() == 2:
                ...
                if self._cos_cache is None:
                    ...
                    self._cos_cache = torch.cat([self._cos_cache, cos[0, ...]],
                                                dim=0)
                    self._sin_cache = torch.cat([self._sin_cache, sin[0, ...]],
                                                dim=0)
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            if using_rope:
                rerotation_cos, rerotation_sin = self._get_rerotation_cos_sin(
                    key_states, self._cos_cache[:self.window_length],
...
class StaticCache(Cache):
    ...
    def __init__(
        ...
        cache_shape = (self.batch_size, self.num_key_value_heads,
        ...
        for idx in range(config.num_hidden_layers):
            if layer_device_map is not None:
                ...
                layer_device = device
            new_layer_key_cache = torch.zeros(cache_shape,
                                              dtype=self.dtype,
                                              device=layer_device)
            new_layer_value_cache = torch.zeros(cache_shape,
                                                dtype=self.dtype,
                                                device=layer_device)
            # Notes:
    ...
    def update(
        ...
        if cache_position is None:
            ...
            try:
                k_out.index_copy_(2, cache_position.to(k_out.device),
                                  key_states.to(k_out.device))
                v_out.index_copy_(2, cache_position.to(v_out.device),
                                  value_states.to(v_out.device))
...
class SlidingWindowCache(StaticCache):
    ...
    def __init__(
        ...
        if not hasattr(config,
                       "sliding_window") or config.sliding_window is None:
            raise ValueError(
    ...
    def update(
        ...
        slicing = torch.ones(self.max_cache_len,
                             dtype=torch.long,
                             device=value_states.device).cumsum(0)
...
class EncoderDecoderCache(Cache):
    ...
    def __init__(self, self_attention_cache: Cache,
    ...
    def from_legacy_cache(
        ...
        if past_key_values is not None:
            for layer_idx in range(len(past_key_values)):
                ...
                cache.self_attention_cache.update(key_states, value_states,
                                                  layer_idx)
                if len(past_key_values[layer_idx]) > 2:
                    key_states, value_states = past_key_values[layer_idx][2:]
                    cache.cross_attention_cache.update(key_states,
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        if self.self_attention_cache.key_cache == []:
            return 0
        if len(self.self_attention_cache.key_cache
               ) > 1 and self.self_attention_cache.key_cache[layer_idx] == []:
            return 0
        return (self.self_attention_cache.key_cache[layer_idx][0, 0].any(
            dim=-1)).sum()
    ...
    def check_dynamic_cache(self, method: str):
        if not (isinstance(self.self_attention_cache, DynamicCache)
    ...
    def batch_split(self, full_batch_size: int,
                    split_size: int) -> "List[EncoderDecoderCache]":
        """Split the current instance into a list of `DynamicCache` by the batch size. This will be used by
        `_split_model_inputs()` in `generation.utils`"""
    ...
    def from_batch_splits(
            cls, splits: List["EncoderDecoderCache"]) -> "EncoderDecoderCache":
        """This is the opposite of the above `batch_split()` method. This will be used by `stack_model_outputs` in
        `generation.utils`"""
        self_attention_cache = DynamicCache()
        cross_attention_cache = DynamicCache()
        for idx in range(len(splits[0])):
            layer_keys = torch.cat([
                current.self_attention_cache.key_cache[idx]
                for current in splits
            ],
                                   dim=0)
            layer_values = torch.cat([
                current.self_attention_cache.value_cache[idx]
                for current in splits
            ],
                                     dim=0)
            ...
            layer_keys = torch.cat([
                current.cross_attention_cache.key_cache[idx]
                for current in splits
            ],
                                   dim=0)
            layer_values = torch.cat([
                current.cross_attention_cache.value_cache[idx]
                for current in splits
            ],
                                     dim=0)
...
class HybridCache(Cache):
    ...
    def __init__(
        ...
        self.num_key_value_heads = (config.num_attention_heads
                                    if config.num_key_value_heads is None else
                                    config.num_key_value_heads)
        ...
        self.key_cache: List[torch.Tensor] = []
        self.value_cache: List[torch.Tensor] = []
        global_cache_shape = (self.batch_size, self.num_key_value_heads,
                              max_cache_len, self.head_dim)
        sliding_cache_shape = (
            self.batch_size,
        ...
        for i in range(config.num_hidden_layers):
            ...
            cache_shape = global_cache_shape if not self.is_sliding[
                i] else sliding_cache_shape
            new_layer_key_cache = torch.zeros(cache_shape,
                                              dtype=self.dtype,
                                              device=layer_device)
            new_layer_value_cache = torch.zeros(cache_shape,
    ...
    def _sliding_update(self, cache_position, layer_idx, key_states,
                        value_states, k_out, v_out, max_cache_len):
        ...
        slicing = torch.ones(max_cache_len,
                             dtype=torch.long,
                             device=value_states.device).cumsum(0)
    ...
    def _static_update(self, cache_position, layer_idx, key_states,
                       value_states, k_out, v_out, max_cache_len):
        k_out[:, :, cache_position] = key_states
        v_out[:, :, cache_position] = value_states
...
class MambaCache:
    ...
    def update_conv_state(self, layer_idx: int, new_conv_state: torch.Tensor,
                          cache_position: torch.LongTensor) -> torch.Tensor:
...
class OffloadedStaticCache(StaticCache):
    ...
    def __init__(
        ...
        head_dim = config.head_dim if hasattr(
            config,
            "head_dim") else config.hidden_size // config.num_attention_heads

        num_key_value_heads = (config.num_attention_heads
                               if config.num_key_value_heads is None else
                               config.num_key_value_heads)

        cache_shape = (max_batch_size, num_key_value_heads, self.max_cache_len,
                       head_dim)
    ...
    def update(
        ...
        cache_position = cache_kwargs.get(
            "cache_position") if cache_kwargs is not None else None
        if cache_position is None:
            k_out.copy_(key_states)
            v_out.copy_(value_states)
            ...
            if layer_idx == 0:
                self.key_cache[layer_idx].copy_(
                    key_states.to(self.offload_device))
                self.value_cache[layer_idx].copy_(
                    value_states.to(self.offload_device))
        else:
            # Note: here we use `tensor.index_copy_(dim, index, tensor)` that is equivalent to
    ...
    def _prefetch_layer_in_context(self, layer_idx: int) -> None:
        ...
        self._device_key_cache[layer_idx & 1].copy_(self.key_cache[layer_idx],
                                                    non_blocking=True)
