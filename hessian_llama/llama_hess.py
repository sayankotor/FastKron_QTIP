...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, mode):
        gate_out = self.gate_proj(x) if type(
            self.gate_proj) == nn.Linear else self.gate_proj(x, mode)
        up_out = self.up_proj(x) if type(
            self.up_proj) == nn.Linear else self.up_proj(x, mode)

        down_in = self.act_fn(gate_out) * up_out
        down_out = self.down_proj(down_in) if type(
            self.down_proj) == nn.Linear else self.down_proj(down_in, mode)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if type(self.q_proj) == nn.Linear:
            query_states = self.q_proj(hidden_states).view(
                hidden_shape).transpose(1, 2)
        else:
            query_states = self.q_proj(hidden_states,
                                       mode).view(hidden_shape).transpose(
                                           1, 2)

        if type(self.k_proj) == nn.Linear:
            key_states = self.k_proj(hidden_states).view(
                hidden_shape).transpose(1, 2)
        else:
            key_states = self.k_proj(hidden_states,
                                     mode).view(hidden_shape).transpose(1, 2)

        if type(self.v_proj) == nn.Linear:
        ...
        if type(self.o_proj) == nn.Linear:
            attn_output = self.o_proj(attn_output)
        else:
            attn_output = self.o_proj(attn_output, mode)
        return attn_output, attn_weights
    ...
    def forward(
        ...
        **kwargs,
        ...
        if start_positions is not None and end_positions is not None:
            loss = self.loss_function(start_logits, end_logits,
